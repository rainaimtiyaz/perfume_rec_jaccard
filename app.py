...
st.markdown("""
Temukan aroma yang sempurna untuk Anda berdasarkan preferensi pribadi! ✨
""")
...
with col_data:
    with st.expander("📊 Tentang Dataset", expanded=True):
        st.markdown(f"""
        - Total Parfum: {len(recommender.df)}
        - Parfum Wanita: {len(recommender.df[recommender.df['Gender'] == 'wanita'])}
        - Parfum Pria: {len(recommender.df[recommender.df['Gender'] == 'pria'])}
        - Parfum Unisex: {len(recommender.df[recommender.df['Gender'] == 'unisex'])}
        """)
        ...
        with st.spinner('🔍 Mencari rekomendasi terbaik untuk Anda...'):
            ...
            if recommendations is None or recommendations.empty:
                ...
                for idx, row in recommendations.iterrows():
                    with st.expander(f"✨️{row['Brand']} - {row['Perfume Name']} (Skor kesamaan Jaccard: {row['similarity']:.2f})"):
                        st.markdown(f"""
